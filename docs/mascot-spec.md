...
## 세모 (삼각형)
- 몸: 흰 삼각형 `fill="#ffffff"`
- 머리: 꼭짓점에서 위로 뻗는 **짧은 직선 3가닥(부챗살 모양)**, stroke #e11d2a w3.
  ⚠️ 금지: W/V 지그재그 폴리라인, 곱슬 곡선 — 반드시 독립된 직선 3개.
- 팔: 검정(#1f2937) 선 + 팔 끝에 **검정 점 손** (원 r≈4, fill #1f2937)
- 다리: 검정 선 2개 + 다리 끝에 **검정 점/타원 발** (ellipse rx≈6 ry≈3.5, fill #1f2937)
  → 손과 발 모두 '검정 점' 모양으로 마감. 선만 끝나는 팔다리 금지.

## 네모 (사각형)
- 몸: 연회색 둥근 사각형 `fill="#f3f4f6"` rx≈10
- 머리: **윗변의 한쪽(왼쪽) 지점에서 시작해 머리 위에 얹히는 둥근 곱슬 3봉우리**.
  기준 곡선: `M(윗변 왼쪽 1/6 지점) q6 -12 12 -4 q6 -12 12 -4 q6 -10 12 -2` 를 캐릭터 크기에 비례 스케일.
  ⚠️ 금지: 뾰족한 지그재그(직선 l 연결), 좌우대칭 왕관형(WW), 중앙 시작형 — 반드시 한쪽에서 시작하는 둥근 q-곡선.
- 팔: **빨간색**(#e11d2a) 곡선
- 다리: **빨간색** 곡선 + **빨간 둥근 사각형 발** (rect rx3, 살짝 회전)

## 동그라미 (원)
- 몸: 흰 원 `fill="#ffffff"`
- 머리: 정수리에 **한 번 꼬인 곱슬 한 가닥**.
  기준 곡선: `M(정수리) q-6 -12 4 -16 q-8 0 -2 8` 를 비례 스케일 — 두 개의 q로 '꼬임'이 생겨야 함.
  ⚠️ 금지: 단순 갈고리/물음표/한 번 휘는 호 — 반드시 두 곡선으로 꼬인 형태.
- 팔: 검정 선 + **검정 점 손** (세모와 동일)
- 다리: 검정 선 2개 + **검정 타원 발** (세모와 동일)
