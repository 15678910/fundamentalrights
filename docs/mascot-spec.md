...
## 공통
- **팔·다리의 시작점은 몸 외곽선 '위'에 있어야 한다.** 세모는 삼각형 빗변 위, 네모는 사각형 변 위,
  동그라미는 원 둘레 위의 점에서 시작한다. 몸 안쪽에서 시작해 몸통을 가로지르는 팔 금지.
- **컷 번호 배지(빨간 원 + 숫자 1~4) 사용 금지** — 기존 패널에서 발견 시 삭제.
...
- 팔: **빨간색**(#e11d2a) 곡선 — 몸 좌우에서 하나씩, **항상 2개**.
  기준: `M(몸 왼쪽변) q-12 2 -16 12` / `M(몸 오른쪽변) q12 2 16 12` 비례 (포즈에 따라 방향 변형 가능).
  ⚠️ 팔 끝에 **손 점(원)을 붙이지 않는다** — 곡선이 그대로 끝나는 것이 원본. 팔 끝의 점/원은 발견 즉시 삭제.
  ⚠️ 팔이 아예 없는 네모 금지 — 없으면 포즈에 맞게 양팔을 추가.
- 다리: **빨간색** 곡선 + **빨간 둥근 사각형 발** (rect rx3, 살짝 회전) — 발은 다리에만 있다.
